...
Then start Claude Code and confirm both appear in the available agent list. They carry the anti-fabrication rules, blocked-source ladder, API-first source list, and budget-triage protocol that the research depends on — the skill's briefs assume those are already in force and don't restate them.
...
| **`references/matrices.md`** | Exact column specifications for every comparison table — flights, lodging, food, day trips, distances, budget, points, discounts. **The columns here are the columns that must appear in the deliverable.** |
| **`references/deliverable.md`** | Report structure, section by section, plus the publishing rules — navigation, wide-table layout, theme handling, what must never be silently dropped. |
...
4. **Research fan-out.** Twelve tracks dispatched to the two agents **in waves of 4–6**, ordered so air and entry/health land first.
...
6. **Synthesis.** Matrices built to spec, contradictions between tracks surfaced rather than averaged away, every matrix closed with a pick.
7. **Deliverable.** Published as an artifact with a sticky section rail, wide scrolling matrices, a risk register, a booking checklist ordered by deadline, and an explicit list of what couldn't be verified.
...
- **Agents never hold an API key.** The orchestrator calls and passes results down, so no credential reaches a subagent prompt or the transcript. Verification then happens once, centrally, instead of twelve times inconsistently.
