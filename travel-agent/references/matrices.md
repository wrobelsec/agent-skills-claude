...
1. **Identity** — what this row is. Pinned when the table scrolls. **Where the row is a physical place, the name itself is a Google Maps link** — see below.
...
### Every place name is its own map link

**Wherever a named physical place appears — a hotel, restaurant, museum, station, trailhead, circuit, meeting point — the name is the anchor.** Not a separate "Map" column, not a pin icon, not a parenthetical link after the name. The reader scanning the identity column can open any row's location directly, which is what they want on a phone standing in the street.

```
place_id known   https://www.google.com/maps/search/?api=1&query=<name>&query_place_id=<id>
coords only      https://www.google.com/maps/search/?api=1&query=<lat>,<lng>
a journey        https://www.google.com/maps/dir/?api=1&origin=<a>&destination=<b>
```

**Prefer the `place_id` form.** A name-only query resolves to whichever branch Google prefers, and that has already put the wrong branch of a four-branch restaurant, and a café in the wrong neighbourhood, into a published report. `references/scripts/places.py` emits a ready-made `maps_url` per venue during verification, so synthesis pastes rather than constructs.

**The city passed to the verifier is the venue's own city, and it is an assertion rather than a hint.** Run `places.py --expect <region>` once per location section: any venue that resolves outside that region is mis-filed, and the script exits with the count.

Two failures make this necessary, and the second is caused by the first. **Thirteen restaurants from one city once shipped inside another city's section** — a combined track returned one table covering three cities and the split went by row order. Every row was true; only the filing was wrong, which is invisible on review. And because the verifier appends the section's city to each lookup, those rows were then queried against the *wrong* city — dragging the geocoder so a mis-filed venue also got a mis-placed pin. **Fix the filing before re-querying**, never the other way round.

**The map link and the action link are different links, and both are required.** The name answers *where is this*; the action-link column answers *how do I book it*. Collapsing them loses one of the two things every row needs — a booking page does not show you the walk from the station, and a map pin does not hold a reservation.

**Never bold a place name in a matrix. The link is the emphasis.**

This is a formatting rule with a real bug behind it. When names were written as `<b>Name</b>`, the pass that injects map links could not match them, and **entire sections shipped with no map links at all** while every other check reported clean. Bolding also competed with the link styling and made the identity column louder than the data.

So: in an identity column a place name is **a link and nothing else**. Bold is for the finding inside a cell — a closure day, a cash-only warning, a verdict — never for the name of the thing the row is about.

**Link the city name once per city, in the first section of its group.** Repeating it in every subsection header is noise; the reader has already been told where that city is. New major locations — a city, a region — each get their own single link when they first appear.

### Fields that come free from the places verification

`places.py` returns considerably more than existence, and **every field below is a column, not a footnote**, because each one replaces something previously left `UNVERIFIED`:

| Column | Source field | Why it earns a column |
|---|---|---|
| **Hours** | `regularOpeningHours` | Compressed to the typical window **plus the closed days**. Closure days have ruined more itineraries than any other single fact |
| **Typical spend** | `priceRange`, else `priceLevel` | `priceRange` returns **real amounts in the local currency**. `priceLevel` is a relative label and **must never be printed as money** — see below |
| **Rating** | `rating` + `userRatingCount` | The count matters as much as the score — 4.6 from 12 reviews is not 4.6 from 12,000 |
| **Step-free** | `accessibilityOptions` | Wheelchair entrance, restroom, seating and parking. **This is the field the per-traveller mobility record needs**, and it is the first source that answers it with anything but a guess |
| **Payment** | `paymentOptions` | `acceptsCashOnly` independently confirms cash-only venues rather than repeating a guidebook claim |

Also available and worth pulling where relevant: `editorialSummary`, `reservable`, `goodForGroups`, `servesVegetarianFood`, `takeout`, `dineIn`, and up to five full `reviews`.

**These apply to lodging exactly as to venues.** A hotel has a rating, a step-free entrance and a payment policy, and those belong in the lodging matrix beside the bed configuration — accessibility especially, since it is the field most likely to eliminate a property outright for a given traveller.

**What this does not replace:** `priceRange` is what other diners report spending, not the operator's rate card. It is a good answer to "what does dinner cost here" and not an answer to "what will this room cost on my dates". Keep the operator's own page as the authority for anything you are booking.

#### `priceLevel` is ordinal and locale-relative — never convert it to a price

Google documents no mapping from `PRICE_LEVEL_MODERATE` and its siblings to currency, and measuring this skill's own verified data shows why no such mapping could exist:

| Enum | Observed `priceRange` for the same venues |
|---|---|
| `PRICE_LEVEL_INEXPENSIVE` | 1 – 2,000 local units |
| `PRICE_LEVEL_MODERATE` | **1,000 – 8,000** across 22 venues — an 8× spread |
| `PRICE_LEVEL_EXPENSIVE` | 15 – 25 — *in a second, cheaper market* |
| `PRICE_LEVEL_VERY_EXPENSIVE` | 10,000 |

Two things follow, and both are fatal to converting it:

- **The bands are not disjoint.** One `INEXPENSIVE` venue sits in the same `JPY 1,000–2,000` band as sixteen `MODERATE` ones. The enum is not even monotonic against the money in practice.
- **It is relative to the local market, so it does not compare across countries.** In the run above, `EXPENSIVE` in the cheaper market converted to *less* than the top of `MODERATE` in the pricier one. A single global scale would have to be wrong in at least one of the two places.

#### Order of preference for a spend cell — the band is genuinely last

1. **The real `priceRange`, in both currencies** — `¥1,000–2,000` with `$6.29–12.58` beneath it, using the dated FX rate. This is the standing both-currencies rule; a spend column is not exempt from it.
2. **Local currency alone**, if the FX lookup failed. Say the conversion is missing rather than dropping the figure.
3. **Only then the ordinal band** — `LOW` / `MID` / `HIGH` / `TOP`, marked *band only — no figure published*.

**Never show the band beside a real figure.** The band is not computed from the money — it is Google's own ordinal judgement relabelled — so printing both invites the reader to think one was derived from the other. They can also flatly disagree: one `INEXPENSIVE` venue and sixteen `MODERATE` ones share the same `JPY 1,000–2,000` range in this dataset.

**Colour the band, because it is the one thing in the column a reader can scan without reading:** green for `Free`/`LOW`, amber for `MID`, red for `HIGH`/`TOP`.

**Also note it is frequently absent.** In this skill's data, **38 of 66 venues returned no `priceLevel` at all — including every hotel and every attraction.** For lodging the field is simply not populated, so a room rate always comes from the operator.

**And `endPrice` is exclusive** while `startPrice` is inclusive: a reported `1,000–2,000` means at least 1,000 and under 2,000.

### How many options per category

**Five is a hard minimum. Ten is a soft cap.**

Below five, a section is a list of whatever the agent happened to find rather than a comparison anyone can choose from — and with four options, one closure or one sell-out leaves three. Above ten it stops being scannable and the picks get lost among the also-rans.

**Where a sweep turns up a couple of extras that genuinely earn their place, keep them** — the cap is soft precisely so a good find is not discarded to satisfy a number. What the cap rules out is padding a table to look thorough.
